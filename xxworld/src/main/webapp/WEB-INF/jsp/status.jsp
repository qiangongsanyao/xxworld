...
<body  class="index">
<p>
<c:if test="${ uuid != null && role != null }">
<c:set var="urltail" value="uuid=${ uuid }&t=${ t }" />
	<%@include file="top"%>
	名字:<c:if test="${ role.sex == 'male' }">♂</c:if><c:if test="${ role.sex == 'female' }">♀</c:if>${ role.name }<br/>
	等级:${ role.level.level }<br/>
	经验:${ role.level.exp }/${ role.level.maxExp }<br/>
	<br/>
	<%@include file="bottom"%>
